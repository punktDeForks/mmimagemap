...
use MikelMade\Mmimagemap\Domain\Repository\AreaRepository;
use MikelMade\Mmimagemap\Domain\Repository\BcolorsRepository;
use MikelMade\Mmimagemap\Domain\Repository\ContentpopupRepository;
use MikelMade\Mmimagemap\Domain\Repository\MapRepository;
use MikelMade\Mmimagemap\Domain\Repository\PointRepository;
...
class BackendController extends ActionController
{
    ...
    */
    public function listAction()
    {
        if (!isset($_GET['id'])) {
            ...
            if (isset($_GET['tx__file_mmimagemapmod1']['path'])) {
                $path = $_GET['tx__file_mmimagemapmod1']['path'];
            }
    ...
    */
    public function addmapAction()
    {
        ...
        if (isset($_POST)) {
            ...
            $name = $_POST['name'];
            if (strlen($name) == 0) {
                $name = $this->mapRepository->GenerateMapName();
            }
    ...
    */
    public function editAction()
    {
        ...
        if (! isset($_SESSION['mmim_blink'])) {
            $_SESSION['mmim_blink'] = 1;
        }
        if (!isset($_SESSION['mmim_close'])) {
            $_SESSION['mmim_close'] = 0;
        }
        if (!isset($_SESSION['mmim_spoint'])) {
            $_SESSION['mmim_spoint'] = 0;
        }
        if (!isset($_SESSION['mmim_epoint'])) {
            $_SESSION['mmim_epoint'] = 0;
        }
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'addarea') {
            ...
            if (!isset($_POST['close'])) {
                ...
                $_SESSION['mmim_close'] = (int)$_POST['close'];
            }
            if (!isset($_POST['spoint'])) {
                $_SESSION['mmim_spoint'] = 0;
            } else {
                $_SESSION['mmim_spoint'] = (int)$_POST['spoint'];
            }
            if (!isset($_POST['epoint'])) {
                $_SESSION['mmim_epoint'] = 0;
            } else {
                $_SESSION['mmim_epoint'] = (int)$_POST['epoint'];
            }
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'addcolor') {
            if (strlen($_POST['be_col']) > 0) {
                if (preg_match('/^[a-f0-9]{6}$/i', $_POST['be_col'])) {
                    ...
                    if (strlen($_POST['be_colname']) == 0) {
                        $_POST['be_colname'] = $_POST['be_col'];
                    }
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'savearea') {
            ...
            if (strlen($_POST['cbid']) > 0) {
                ...
                if ($new == true) {
                    $this->contentpopupRepository->add($cbox);
                } else {
                    $this->contentpopupRepository->update($cbox);
                }
            }
